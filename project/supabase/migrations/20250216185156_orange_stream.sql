/*
  ...
  1. Tables
    - todo_profiles: Public user profiles with roles
    - todo_folders: Organizes todos into shareable folders
    - todos: Main todo items with sharing capabilities
...
CREATE TABLE IF NOT EXISTS todo_profiles (
  ...
  CONSTRAINT todo_profiles_role_check CHECK (role IN ('admin', 'user'))
);
