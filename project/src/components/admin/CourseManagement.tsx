...
import { PlusCircle, Edit, Trash2, Users, Video, Award } from 'lucide-react';
...
export default function CourseManagement() {
  ...
  const [showDirectCertificateModal, setShowDirectCertificateModal] = useState(false);
    ...
    <div className="space-y-6">
      {selectedCourse ? (
        <div>
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Course #{selectedCourse.course_number}
              </p>
            </div>
            <div className="flex items-center gap-4">
              <button
                onClick={() => setShowDirectCertificateModal(true)}
                className="inline-flex items-center px-3 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700"
              >
                <Award className="h-4 w-4 mr-2" />
                Issue Direct Certificate
              </button>
              <button
          ...
          {showDirectCertificateModal && (
            <IssueCertificateModal
              course={selectedCourse}
              onClose={() => setShowDirectCertificateModal(false)}
              onSuccess={() => {
                toast.success('Certificate issued successfully');
                setShowDirectCertificateModal(false);
              }}
            />
          )}
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="px-6 py-3 text-left text -left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap min-w-[120px]">
                      Course Number
                    </th>
